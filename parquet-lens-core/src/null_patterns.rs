...
pub fn analyze_null_patterns(agg_stats: &[AggregatedColumnStats]) -> Vec<NullPatternGroup> {
    ...
    let always_null: Vec<String> = agg_stats
        .iter()
    ...
    if !always_null.is_empty() {
        groups.push(NullPatternGroup {
            ...
            pattern_type: "always_null".into(),
        });
    }
    // never-null columns (0%)
    let never_null: Vec<String> = agg_stats
        .iter()
    ...
    if never_null.len() > 1 {
        groups.push(NullPatternGroup {
            columns: never_null,
            null_percentage: 0.0,
            pattern_type: "never_null".into(),
        });
    }
    // correlated-null groups: same null rate within 2%, >1% null, not in always_null
    let candidates: Vec<&AggregatedColumnStats> = agg_stats
        .iter()
