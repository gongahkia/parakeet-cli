...
pub fn aggregate_column_stats(per_rg: &[ColumnStats], total_rows: i64) -> Vec<AggregatedColumnStats> {
...
pub fn analyze_uniformity(profiles: &[RowGroupProfile]) -> UniformityReport {
    ...
    if n == 0 {
        return UniformityReport {
            count: 0, mean_rows: 0.0, median_rows: 0.0, stddev_rows: 0.0,
            min_rows: 0, max_rows: 0, mean_bytes: 0.0, median_bytes: 0.0,
            stddev_bytes: 0.0, min_bytes: 0, max_bytes: 0, outlier_indices: Vec::new(),
        };
    }
    ...
    let stddev_bytes = (bytes.iter().map(|b| (b - mean_bytes).powi(2)).sum::<f64>() / n as f64).sqrt();
    let mut sorted_rows = rows.clone(); sorted_rows.sort_by(f64::total_cmp);
    let mut sorted_bytes = bytes.clone(); sorted_bytes.sort_by(f64::total_cmp);
    let outlier_indices = profiles.iter().enumerate()
    ...
    UniformityReport {
        ...
        stddev_bytes,
        min_bytes: profiles.iter().map(|p| p.total_byte_size).min().unwrap_or(0),
        max_bytes: profiles.iter().map(|p| p.total_byte_size).max().unwrap_or(0),
        outlier_indices,
    }
}
...
pub fn analyze_encodings(meta: &ParquetMetaData) -> Vec<EncodingAnalysis> {
    ...
    map.into_iter().map(|(name, mut encs)| {
        encs.sort(); encs.dedup();
        let is_plain_only = encs.iter().all(|e| e == "PLAIN" || e == "RLE_DICTIONARY" || e == "BIT_PACKED");
        let is_plain_only = encs == vec!["PLAIN".to_string()];
        EncodingAnalysis { column_name: name, encodings: encs, is_plain_only }
    }).collect()
}
...
pub fn analyze_compression(meta: &ParquetMetaData) -> Vec<CompressionAnalysis> {
    ...
    map.into_iter().map(|(name, (codec, uncomp, comp))| {
        let compression_ratio = if comp > 0 { uncomp as f64 / comp as f64 } else { 1.0 };
        let is_uncompressed = codec == "UNCOMPRESSED";
        CompressionAnalysis { column_name: name, codec, uncompressed_size: uncomp, compressed_size: comp, compression_ratio, is_uncompressed }
    }).collect()
}
