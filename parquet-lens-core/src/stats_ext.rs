...
pub fn analyze_partitions(paths: &[ParquetFilePath]) -> Vec<PartitionInfo> {
    ...
    for pf in paths {
        ...
        let row_count = open_parquet_file(&pf.path).map(|(info, _)| info.row_count).unwrap_or(0);
        for (k, v) in &pf.partitions {
            let entry = key_values.entry(k.clone()).or_default().entry(v.clone()).or_insert((0, 0));
            entry.0 += row_count;
            entry.1 += file_size;
        }
    }
    key_values.into_iter().map(|(key, value_map)| {
        let mut distinct_values: Vec<String> = value_map.keys().cloned().collect();
        distinct_values.sort();
        let partition_row_counts: HashMap<String, i64> = value_map.iter().map(|(v, (r, _))| (v.clone(), *r)).collect();
        let partition_byte_sizes: HashMap<String, u64> = value_map.iter().map(|(v, (_, b))| (v.clone(), *b)).collect();
        let row_counts: Vec<i64> = partition_row_counts.values().cloned().collect();
        let median_rows = {
            let mut sorted = row_counts.clone(); sorted.sort();
            if sorted.is_empty() { 1 } else { sorted[sorted.len()/2].max(1) }
        };
        let skewed_partitions = partition_row_counts.iter()
            .filter(|(_, &r)| r > median_rows * 3)
            .map(|(v, _)| v.clone()).collect();
        PartitionInfo { key, distinct_values, partition_row_counts, partition_byte_sizes, skewed_partitions }
    }).collect()
}
...
pub fn compute_correlation(meta: &ParquetMetaData, path: &Path) -> Result<CorrelationMatrix> {
    ...
    let builder = ParquetRecordBatchReaderBuilder::try_new(file).map_err(ParquetLensError::Parquet)?;
    let schema = builder.schema().clone();
    let numeric_cols: Vec<usize> = schema.fields().iter().enumerate().filter_map(|(i, f)| match f.data_type() {
        DataType::Int32|DataType::Int64|DataType::Float32|DataType::Float64 => Some(i),
        _ => None,
    }).collect();
    let col_names: Vec<String> = numeric_cols.iter().map(|&i| schema.field(i).name().clone()).collect();
    let n = numeric_cols.len();
    if n == 0 { return Ok(CorrelationMatrix { columns: Vec::new(), values: Vec::new() }); }
    // accumulators: sum, sum_sq, sum_cross, count
    let mut sums = vec![0.0f64; n];
    let mut sums_sq = vec![0.0f64; n];
    let mut cross = vec![vec![0.0f64; n]; n];
    let mut count = 0u64;
    let mask = parquet::arrow::ProjectionMask::roots(builder.parquet_schema(), numeric_cols.clone());
    let reader = builder.with_projection(mask).with_batch_size(65536).build().map_err(ParquetLensError::Parquet)?;
    for batch_result in reader {
        let batch = batch_result.map_err(|e| ParquetLensError::Arrow(e))?;
        let batch_n = batch.num_rows();
        let vals: Vec<Vec<f64>> = (0..n).map(|ci| {
            let col = batch.column(ci);
            (0..batch_n).map(|row| if col.is_null(row) { f64::NAN } else {
                match col.data_type() {
                    ...
                    DataType::Float64 => col.as_any().downcast_ref::<Float64Array>().unwrap().value(row) as f64,
                    _ => f64::NAN,
                }
            }).collect()
        }).collect();
        for row in 0..batch_n {
            ...
            for i in 0..n {
                let v = vals[i][row];
                if !v.is_nan() {
                    sums[i] += v; sums_sq[i] += v * v;
                    for j in i..n { if !vals[j][row].is_nan() { cross[i][j] += v * vals[j][row]; } }
                }
            }
        }
    }
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            matrix[i][j] = if si > 0.0 && sj > 0.0 { cov / (si * sj) } else { 0.0 };
        }
    }
    Ok(CorrelationMatrix { columns: col_names, values: matrix })
}
...
pub fn string_length_histogram(path: &Path, column: &str, bins: usize) -> Result<StringLengthHist> {
    use parquet::arrow::arrow_reader::ParquetRecordBatchReaderBuilder;
    use arrow::array::{StringArray, LargeStringArray};
    use arrow::datatypes::DataType;
    use parquet_lens_common::ParquetLensError;
    let file = std::fs::File::open(path)?;
    let builder = ParquetRecordBatchReaderBuilder::try_new(file).map_err(ParquetLensError::Parquet)?;
    let reader = builder.with_batch_size(65536).build().map_err(ParquetLensError::Parquet)?;
    let mut lengths = Vec::new();
    for batch in reader {
        let batch = batch.map_err(|e| ParquetLensError::Arrow(e))?;
        let col_idx = batch.schema().fields().iter().position(|f| f.name() == column);
        if let Some(idx) = col_idx {
            let col = batch.column(idx);
            for row in 0..batch.num_rows() {
                if !col.is_null(row) {
                    let len = match col.data_type() {
                        DataType::Utf8 => col.as_any().downcast_ref::<StringArray>().map(|a| a.value(row).len()),
                        DataType::LargeUtf8 => col.as_any().downcast_ref::<LargeStringArray>().map(|a| a.value(row).len()),
                        _ => None,
                    };
                    if let Some(l) = len { lengths.push(l); }
                }
            }
        }
    }
    if lengths.is_empty() { return Ok(StringLengthHist { column_name: column.to_owned(), bins: Vec::new() }); }
    ...
    let result = counts.into_iter().enumerate().map(|(i, c)| (min + i * width, min + (i+1) * width, c)).collect();
    Ok(StringLengthHist { column_name: column.to_owned(), bins: result })
}
...
pub fn detect_sort_order(meta: &ParquetMetaData) -> Vec<SortedOrderInfo> {
    ...
    (0..schema.num_columns()).map(|col_idx| {
...
pub fn analyze_page_index(meta: &ParquetMetaData) -> PageIndexInfo {
    ...
    for rg_idx in 0..rg_count {
        ...
        for col_idx in 0..rg.num_columns() {
            total += 1;
            let chunk = rg.column(col_idx);
            if chunk.column_index_offset().is_some() { col_idx_count += 1; }
            if chunk.offset_index_offset().is_some() { off_idx_count += 1; }
        }
    }
    PageIndexInfo {
        has_column_index: col_idx_count > 0,
        has_offset_index: off_idx_count > 0,
        column_index_coverage_pct: if total > 0 { col_idx_count as f64 / total as f64 * 100.0 } else { 0.0 },
    }
}
...
pub fn detect_bloom_filters(meta: &ParquetMetaData) -> Vec<BloomFilterInfo> {
    ...
    (0..schema.num_columns()).map(|col_idx| {
        ...
        BloomFilterInfo { column_name: col_name, has_bloom_filter: has_bloom }
    }).collect()
}
