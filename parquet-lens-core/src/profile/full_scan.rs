...
use super::histogram::{HistogramBin, build_histogram};
use super::string_profiler::StringAccumulator;
use super::temporal::TemporalAccumulator;
use super::boolean::BooleanAccumulator;
use super::{CardinalityEstimate, FrequencyResult, NumericProfile, StringProfile, TemporalProfile, BooleanProfile};
...
pub fn profile_columns_with_timeout(
    ...
    let builder = if let Some(cols) = columns {
        let indices: Vec<usize> = cols.iter().filter_map(|c| {
            schema.fields().iter().position(|f| f.name() == c)
        }).collect();
    ...
    let reader = builder.with_batch_size(batch_size)
        .build()
        .map_err(ParquetLensError::Parquet)?;

    let field_names: Vec<String> = reader.schema().fields().iter().map(|f| f.name().clone()).collect();
    ...
    let mut numeric_accs: Vec<Option<NumericAccumulator>> = reader.schema().fields().iter()
    ...
    let mut str_accs: Vec<Option<StringAccumulator>> = reader.schema().fields().iter()
        .map(|f| match f.data_type() {
            DataType::Utf8|DataType::LargeUtf8 => Some(StringAccumulator::new()),
            _ => None,
        }).collect();
    let mut temporal_accs: Vec<Option<TemporalAccumulator>> = reader.schema().fields().iter()
        .map(|f| match f.data_type() {
            DataType::Timestamp(_,_)|DataType::Date32|DataType::Date64 => Some(TemporalAccumulator::new()),
            _ => None,
        }).collect();
    let mut bool_accs: Vec<Option<BooleanAccumulator>> = reader.schema().fields().iter()
        .map(|f| match f.data_type() {
            ...
            for row in 0..col_array.len() {
                ...
                match col_array.data_type() {
                    DataType::Int8 => {
                        ...
                        if let Some(acc) = &mut numeric_accs[col_idx] { acc.add(v); }
                    ...
                    DataType::Utf8 => {
                        ...
                        if let Some(acc) = &mut str_accs[col_idx] { acc.add(a.value(row)); }
                    }
                    DataType::LargeUtf8 => {
                        let a = col_array.as_any().downcast_ref::<LargeStringArray>().unwrap();
                        if let Some(acc) = &mut str_accs[col_idx] { acc.add(a.value(row)); }
                    }
                    DataType::Boolean => {
                        let a = col_array.as_any().downcast_ref::<BooleanArray>().unwrap();
                        if let Some(acc) = &mut bool_accs[col_idx] { acc.add(Some(a.value(row))); }
                    }
                    DataType::Timestamp(TimeUnit::Millisecond, _) => {
                        let a = col_array.as_any().downcast_ref::<TimestampMillisecondArray>().unwrap();
                        if let Some(acc) = &mut temporal_accs[col_idx] { acc.add_ms(a.value(row)); }
                    }
                    DataType::Timestamp(TimeUnit::Second, _) => {
                        let a = col_array.as_any().downcast_ref::<TimestampSecondArray>().unwrap();
                        if let Some(acc) = &mut temporal_accs[col_idx] { acc.add_ms(a.value(row) * 1000); }
                    }
                    DataType::Timestamp(TimeUnit::Microsecond, _) => {
                        let a = col_array.as_any().downcast_ref::<TimestampMicrosecondArray>().unwrap();
                        if let Some(acc) = &mut temporal_accs[col_idx] { acc.add_ms(a.value(row) / 1000); }
                    }
                    DataType::Timestamp(TimeUnit::Nanosecond, _) => {
                        let a = col_array.as_any().downcast_ref::<TimestampNanosecondArray>().unwrap();
                        if let Some(acc) = &mut temporal_accs[col_idx] { acc.add_ms(a.value(row) / 1_000_000); }
                    }
                    DataType::Date32 => {
                        let a = col_array.as_any().downcast_ref::<Date32Array>().unwrap();
                        if let Some(acc) = &mut temporal_accs[col_idx] { acc.add_ms(a.value(row) as i64 * 86400 * 1000); }
                    }
                    DataType::Date64 => {
