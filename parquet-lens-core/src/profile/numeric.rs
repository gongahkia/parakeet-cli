...
pub struct NumericProfile {
    ...
    pub min: f64,
    pub max: f64,
    pub p1: f64, pub p5: f64, pub p25: f64, pub p50: f64,
    pub p75: f64, pub p95: f64, pub p99: f64,
    pub skewness: f64,
    pub kurtosis: f64,
    pub count: u64,
}
...
impl NumericAccumulator {
    pub fn new() -> Self {
        Self {
            ...
            sum: 0.0, sum_sq: 0.0, sum_cube: 0.0, sum_quad: 0.0,
            min: f64::MAX, max: f64::MIN, count: 0,
            values_buf: Vec::new(),
        }
    }
    pub fn add(&mut self, v: f64) {
        ...
        self.count += 1;
        if self.values_buf.len() >= 10000 { self.flush(); }
    }
    fn flush(&mut self) {
        if self.values_buf.is_empty() { return; }
        // merge_unsorted is a method on &self returning a new TDigest
        let merged = self.digest.merge_unsorted(self.values_buf.drain(..).collect());
        self.digest = merged;
    }
    pub fn finish(mut self) -> NumericProfile {
        ...
        if n == 0.0 {
            return NumericProfile { mean:0.0,stddev:0.0,min:0.0,max:0.0,
                p1:0.0,p5:0.0,p25:0.0,p50:0.0,p75:0.0,p95:0.0,p99:0.0,
        ...
        let kurtosis = if variance > 0.0 {
            ((self.sum_quad / n) - 4.0 * mean * (self.sum_cube / n)
             + 6.0 * mean * mean * (self.sum_sq / n) - 3.0 * mean.powi(4)) / variance.powi(2) - 3.0
        } else { 0.0 };
        NumericProfile {
            mean, stddev, min: self.min, max: self.max,
            ...
            skewness, kurtosis, count: self.count,
        }
    }
}
